...
#ifdef _MSC_VER
	#define _CRT_SECURE_NO_WARNINGS		//Added this for fopen_s and scanf_s error
#endif
...
{
	real_world_origin.x = -12.0f;
	real_world_origin.y = -5.0f;
	real_world_origin.z = -5.0f;
...
void render_particles()
{
	...
	for (unsigned int i = 0; i < sph->getpNum(); i++)
	{
		...
		glVertex3f(sph->particles[i].pos.x*sim_ratio.x + real_world_origin.x,
			sph->particles[i].pos.y*sim_ratio.y + real_world_origin.y,
			sph->particles[i].pos.z*sim_ratio.z + real_world_origin.z);
		glEnd();
	}
}
*/
...
void display_func()
{
	...
	sprintf(window_title, "SPH 3D 111987438.");
